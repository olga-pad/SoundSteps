...
function speak(text,rate=.68){if(!('speechSynthesis'in window))return;speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.lang='ru-RU';u.rate=rate;speechSynthesis.speak(u);}
...
async function audioHelp(){if(!current)return;usedAudio=true;const spans=[...$('word').children],letters=[...current[0]];for(let i=0;i<letters.length;i++){spans.forEach(x=>x.style.transform='');spans[i].style.display='inline-block';spans[i].style.transform='scale(1.18)';speak(letters[i],.58);await new Promise(r=>setTimeout(r,650));}spans.forEach(x=>x.style.transform='');await new Promise(r=>setTimeout(r,180));speak(current[0],.62);}
